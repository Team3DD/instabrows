...
const REVIEW_IMAGE = '/images/reviews/review.jpeg'; // Nueva ruta de imagen optimizada
  ...
  const ReviewCard = ({ review }: { review: Review }) => {
    ...
    return (
      ...
      >
        ...
        <div className="mt-auto border-t border-gray-100 p-3 flex items-center justify-center bg-gray-50">
          <button className="text-gray-700 text-xs font-bold hover:text-[#9B8977] transition-colors duration-300 mx-4">Me gusta</button>
          <button className="text-gray-700 text-xs font-bold hover:text-[#9B8977] transition-colors duration-300 mx-4">Comentar</button>
          <button className="text-gray-700 text-xs font-bold hover:text-[#9B8977] transition-colors duration-300 mx-4">Compartir</button>
        </div>
      </motion.div>
    );
  };
  ...
  return (
    <>
      ...
      <Script
        ...
        strategy="lazyOnload"
        onLoad={() => {
          ((window as Window & typeof globalThis & { FB?: { init: (config: { appId: string; version: string; xfbml: boolean }) => void } }).FB)?.init({
            appId: FB_APP_ID,
            version: 'v16.0',
            xfbml: true
          });
